...
<BODY>
	<FORM id="customerForm" name="customerForm"
		action="${pageContext.request.contextPath }/linkman/findByPage.action"
		method=post>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												共[<B>${totalRecord}</B>]条记录,共[<B>${totalPage}</B>]页
												,每页显示
												...
												<c:if test="${pageNumber != 1 }">
													[<A href="javascript:to_page(${pageNumber-1})">前一页</A>]
												</c:if>
												
												
												<!-- 展示所有的页码 -->
												<c:forEach begin="1" end="${totalPage }" var="n">
													<!-- 判断是否是当前页,若是当前页不加超链接;否则添加超链接  -->
													<c:if test="${n == pageNumber }">
														${n }
													</c:if>
													<c:if test="${n != pageNumber }">
														<a href="javascript:to_page(${n })">${n }</a>
													</c:if>
												</c:forEach>
												
												<%-- <B>${page}</B> --%>
												
												<!-- 判断是否是最后一页,若不是则显示 后一页  -->
												
												<c:if test="${pageNumber != totalPage }">
													[<A href="javascript:to_page(${pageNumber+1})">后一页</A>] 
												</c:if>
												到
